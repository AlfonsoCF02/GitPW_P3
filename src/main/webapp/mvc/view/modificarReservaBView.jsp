...
<head>
	<meta charset="UTF-8">
	<title>Modificar Reserva Bono</title>
...
if (customerBean != null && !customerBean.getEmail().equals("") && (customerBean.getPrivilegios().toString().equals("user"))) {%>
	<div id=div_form_modificarResB>
		<form id="formReservaInd" method="post" action="${pageContext.request.contextPath}/modificarreservab">
			<input type="hidden" name="email" value=<%=customerBean.getEmail()%>>
			<label for="idres">
				<span>Reserva</span>
			</label>
			<select id="idres" name="idres">
				<%
